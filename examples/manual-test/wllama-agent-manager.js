...
class WllamaAgentManager {
    ...
    constructor(config = {}) {
        ...
        this.maxHistoryTurns = config.maxHistoryTurns ?? (window.MAX_HISTORY_TURNS ?? 5);
        ...
        this.maxHistoryUserMsgLength = config.maxHistoryUserMsgLength
            ?? (window.MAX_HISTORY_USER_MSG_LENGTH ?? 1500);
    ...
    setToolSchemas(schemas) {
        ...
        console.log(`[WllamaAgent] ${this.toolSchemas.length} tool schema(s) registered`);
    }
    ...
    * @param {string} [opts.hfFile] - GGUF filename (e.g. 'Hermes-2-Pro-Mistral-7B.Q5_K_M.gguf')
    ...
    async initializeModel(opts = {}) {
        ...
        const wasmUrl = window.WLLAMA_WASM_URL || '/static/wasm/single-thread/wllama.wasm';
        ...
        if (!WllamaClass) {
            throw new Error('Wllama not loaded. Ensure wllama-bundle.js is imported.');
        }
        ...
        if (cache) {
            ...
            if (blob) {
                ...
                blob = await this._fetchAndCache(modelUrl, opts.progressCallback, cache);
            }
            ...
            console.warn('[WllamaAgent] No cache manager — falling back to direct HF load');
            if (!opts.hfRepo || !opts.hfFile) {
                throw new Error('No cache manager available and no hfRepo+hfFile provided');
            }
            const progressCb = opts.progressCallback || this._defaultProgressCallback.bind(this);
            await this._loadModelWithGpuFallback(
                (fallbackConfig) =>
    ...
    async _fetchAndCache(url, progressCallback, cache) {
        const progressCb = progressCallback || this._defaultProgressCallback.bind(this);
        ...
        cache.put(url, blob).catch((err) => {
            if (err && err.name === 'QuotaExceededError') {
                ...
                const msg = 'Model too large to cache — it will re-download next time (browser storage quota exceeded)';
                console.error('[WllamaAgent] ❌ ' + msg, err);
                if (this.onStatusChange) {
                    try { this.onStatusChange('cache_quota_exceeded', msg); } catch (_) { /* status UI is best-effort */ }
                }
            } else {
    ...
    async processMessage(userMessage, opts = {}) {
        ...
        const grammarSupported = !this.functionCallingConfig
            || this.functionCallingConfig.architecture === 'grammar-wllama';
        const hasTools = this.toolSchemas.length > 0 && this.toolBridge && grammarSupported;
        ...
        if (hasTools && window.WllamaToolCaller) {
            ...
            if (!this._nativeToolsUnsupported) {
                try {
                    const response = await this._processWithNativeTools(
                        userMessage, messages, sampling, opts
                    );
            ...
            const toolPrompt = window.WllamaToolCaller.buildToolSystemPrompt(this.toolSchemas);
            ...
            let pass1Response;
            try {
                pass1Response = this._extractContent(await this.wllama.createChatCompletion({
                    messages,
            ...
            const toolCall = window.WllamaToolCaller.detectToolCall(pass1Response, this.toolSchemas);
            ...
            if (toolCall) {
                ...
                const synthesisResponse = await this._synthesizeToolResult(
                    userMessage, toolCall, toolResult, {
            ...
            if (this._grammarUnsupported && pass1Response.trim().toLowerCase() !== 'null') {
                console.log('[WllamaAgent] Unconstrained pass returned direct answer — skipping Pass 2');
            ...
            if (opts.onToken) {
                response = await this._streamCompletion(messages, {
                    max_tokens: opts.maxTokens || WllamaAgentManager.MAX_TOKENS_SYNTHESIS_PASS,
                    ...sampling,
                    onNewToken: opts.onToken,
                });
            } else {
                response = this._extractContent(await this.wllama.createChatCompletion({
                    messages,
                    max_tokens: opts.maxTokens || WllamaAgentManager.MAX_TOKENS_SYNTHESIS_PASS,
                    ...sampling,
                }));
    ...
    _addToHistory(userMessage, assistantResponse) {
        ...
        const storedUserMsg = (this.maxHistoryUserMsgLength > 0
    ...
    async _processWithNativeTools(userMessage, messages, sampling, opts) {
        ...
        for (let round = 1; round <= MAX_TOOL_ITERATIONS; round++) {
            ...
            try {
                completion = await this._nativeToolCompletion(
                    nativeMessages, tools, sampling, opts
                );
            } catch (err) {
                ...
                return await this._synthesizeToolResult(
                    userMessage, lastToolCall, lastToolResult, synthesisOpts
                );
            }
            ...
            for (let i = 0; i < toolCalls.length; i++) {
                ...
                console.log('[WllamaAgent] Native tool call:', toolCall.name, toolCall.parameters);
                ...
                nativeMessages.push({
                    ...
                    content: resultStr.length > NATIVE_MAX_RESULT_CHARS
        ...
        console.warn('[WllamaAgent] Native tools hit iteration limit — synthesizing from last tool result');
        return await this._synthesizeToolResult(
            userMessage, lastToolCall, lastToolResult, synthesisOpts
        );
    }
        ...
        for await (const chunk of iter) {
            ...
            if (delta.tool_calls) {
                for (const tc of delta.tool_calls) {
                    ...
                    if (tc.function?.arguments) collected[idx].arguments += tc.function.arguments;
                }
            }
        }
    ...
    async _executeTool(toolCall) {
        ...
        try {
            ...
            if (msg.includes('unexpected keyword argument') || msg.includes('takes 0 positional argument')) {
                console.warn(`[WllamaAgent] Tool "${toolCall.name}" rejected hallucinated args — retrying with no arguments`);
                try {
                    const result = await this.toolBridge.executeTool(toolCall.name, {});
                    console.log(`[WllamaAgent] Tool "${toolCall.name}" retry result:`, result);
                    return result;
                } catch (retryErr) {
                    console.error(`[WllamaAgent] Tool "${toolCall.name}" retry also failed:`, retryErr);
    ...
    async _synthesizeToolResult(userMessage, toolCall, toolResult, opts = {}) {
        const toolFailed = typeof toolResult === 'object' && toolResult !== null && 'error' in toolResult;
        const resultStr = typeof toolResult === 'string'
            ? toolResult
        ...
        const truncatedResult = resultStr.length > MAX_RESULT_CHARS
        ...
        const synthesisMessages = [
            ...
            {
                role: 'user',
                content: 'Please provide a clear, helpful answer in plain natural language. Do not output JSON or tool calls — respond with text only.',
            },
        ];
        ...
        if (trimmed.startsWith('{') && trimmed.includes('"name"')) {
            console.warn('[WllamaAgent] Synthesis produced tool-call JSON — using fallback message');
            return toolFailed
                ? 'I encountered an error while processing your request. Please try rephrasing your question.'
                : `Here are the results:\n${resultStr}`;
        }

        return response || (toolFailed
